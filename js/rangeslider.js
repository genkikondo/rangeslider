...
define(function rangeSlider() {
  ...
  // Allows multiple instances of slider on a page
  var plugin_count = 0;
  ...
  var RangeSlider = function (input, options, plugin_count) {
    ...
    var data = {
      ...
      divots: $inp.data('divots'),
      rings: $inp.data('rings'),
    ...
    // get merge config with defaults
    this.options = $.extend({
      type: 'single',
  ...
  RangeSlider.prototype = {
    ...
    append: function () {
      ...
      this.$cache.divots = this.$cache.cont.find('.irs-divots');
      this.$cache.grid = this.$cache.cont.find('.irs-grid');

      if (this.options.type === 'single') {
        this.$cache.cont.append(single_html);
        this.$cache.s_single = this.$cache.cont.find('.single');
        this.$cache.rings = this.$cache.cont.find('.irs-rings');
    ...
    setMinMax: function () {
      ...
      if (this.options.values.length) {
        this.$cache.min.html(this.decorate(this.options.labels[this.options.min]));
        this.$cache.max.html(this.decorate(this.options.labels[this.options.max]));
      } else {
    ...
    drawRings: function () {
      ...
      for (i = num_rings; i > 0; i--) {
        w_diff = 4 * i - 2;
        p_diff = this.toFixed(((w_diff + 2) / 2) / this.coords.w_rs * 100);
        w_ring = this.coords.w_handle + w_diff;
        p_ring = this.coords.p_single - p_diff;
        f_ring = i * 100;

        html += '<span class="irs-ring" data-left="' + p_ring + '%" data-width="' + w_ring + 'px" data-fade="' + f_ring + '"></span>';
      }

      this.$cache.rings.html(html);
      this.$cache.ring = this.$cache.cont.find('.irs-ring');
      this.$cache.ring.on('touchstart.irs_' + this.plugin_count, this.pointerDown.bind(this, 'single'));
      this.$cache.ring.on('mousedown.irs_' + this.plugin_count, this.pointerDown.bind(this, 'single'));

      var $rings = this.$cache.ring;
      $rings.each(function() {
        var $element = $(this);
        $element.css('left', $(this).data('left'));
        $element.css('width', $(this).data('width'));
        $element.css('height', $(this).data('width'));
        $element.css('border-radius', $(this).data('width'));
      ...
      function animateRings() {
        $rings.each(function() {
          ...
          $element.delay(fade).fadeIn(500).fadeOut(500);
        });
      }
      animateRings();
      setInterval(animateRings, 2000);
    },
    ...
    drawLabels: function () {
      ...
      if (this.options.type === 'single') {
        ...
        if (values_num) {
          text_single = this.decorate(labels[this.result.from]);
        ...
        if (values_num) {
          ...
          if (this.options.decorate_both) {
            ...
            text_single += this.decorate(labels[this.result.to]);
          } else {
            text_single = this.decorate(labels[this.result.from] + this.options.values_separator + labels[this.result.to]);
          }
          text_from = this.decorate(labels[this.result.from]);
          text_to = this.decorate(labels[this.result.to]);
    ...
    decorate: function (num, original) {
      ...
      if (o.max_postfix) {
        if (o.values.length && num === o.labels[o.max]) {
    ...
    appendDivots: function () {
      if (!this.options.divots) {
        return;
      }
      ...
      this.calcDivotsMargin();
      var real_width = 100 - this.coords.p_handle;

      for (i = 0; i < big_num + 1; i++) {

        p_single_real = this.calcWithStep(i / big_num * 100);
        p_single_real = this.checkDiapason(p_single_real, this.options.from_min, this.options.from_max);
        p_single = this.toFixed(p_single_real / 100 * real_width);

        html += '<span class="irs-divot" data-left="' + p_single + '%"></span>';
      }

      this.$cache.divots.html(html);
      this.$cache.divot = this.$cache.cont.find('.irs-divot');
      this.$cache.divot.on('touchstart.irs_' + this.plugin_count, this.pointerClick.bind(this, 'click'));
      this.$cache.divot.on('mousedown.irs_' + this.plugin_count, this.pointerClick.bind(this, 'click'));

      this.$cache.divot.each(function() {
        $(this).css('left', $(this).data('left'));
      });
    },

    calcDivotsMargin: function () {
      this.coords.w_rs = this.$cache.rs.outerWidth(false);
      if (!this.coords.w_rs) {
        return;
      }
    ...
    redrawDivots: function () {
      this.$cache.divots.remove('.irs-divot');
      this.appendDivots();
    },
    ...
    appendGrid: function () {
      ...
      for (i = 0; i < big_num + 1; i++) {
        ...
        for (z = 1; z <= local_small_max; z++) {
          ...
          html += '<span class="irs-grid-pol small" data-left="' + small_w + '%"></span>';
        }

        html += '<span class="irs-grid-pol" data-left="' + big_w + '%"></span>';

        result = this.calcReal(big_w);
        if (o.values.length) {
          result = o.labels[result];
        } else {
          result = this._prettify(result);
        }

        html += '<span class="irs-grid-text js-grid-text-' + i + '" data-left="' + big_w + '%">' + result + '</span>';
      }
